...
private:
  ...
  void animateRunning(const Uint8 *keyState, bool isMoving,
                      float timeBetweenFrames);

  /*----------------Checks for collisions with platforms----------------------*/
  bool collisionUnderPlatform(std::vector<Platform *> &platforms,
                              float timeBetweenFrames);
...
public:
  ...
  void init(SDL_Renderer *renderer, std::string textureFilePath, int numFramesX,
            int numFramesY);
